...
parser.add_argument('-s', '--servers', help='comma seaprated list of host, all compared to first', required=True)
...
def execute(sql=None, host = None,q = None, server=None):
...
#  print (temp)
  if len(temp) > 3:
    ...
    count_results[table] = {}
    sql = "select count(1) from " + temp[1] + "." + temp[2]
    print (sql)
    server_count = 0
    for server in servers:
        ...
        proc = Process(target=execute, args=(sql, host, q,server)) 
        procs.append(proc)
        proc.start()
    for proc in procs:
        proc.join()
