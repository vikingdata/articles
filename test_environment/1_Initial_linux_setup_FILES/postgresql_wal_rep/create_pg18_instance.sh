...
REPL_PASS=`openssl rand -base64 10 | head -c 16 | sed 's/[^[:alnum:]_-]/\_/' | sed 's/[^[:alnum:]_-]/\-/g'`
REINIT=0
...
wal_level = logical
max_wal_senders = 10
synchronized_standby_slots = '\''standby_b'\''
...
# Always add user. Id subscriber, its data will be erased when setup as a subscriber. 
    ...
    sql="CREATE ROLE $REPL_USER2 WITH REPLICATION LOGIN PASSWORD '$REPL_PASS'"
    sudo -u postgres psql -p $PORT -c "$sql;"
   ...
   FROM pg_roles   where rolname not like 'pg_%'  ORDER BY rolname;"
    ...
    sql="CREATE TABLE wal1 (id INT GENERATED by default AS IDENTITY PRIMARY KEY);"
    sudo -u postgres psql -d  replication -p $PORT  -P pager=off -c "$sql;"

    sql="CREATE TABLE logical1 (id INT GENERATED by default AS IDENTITY PRIMARY KEY);"
    sudo -u postgres psql -d  replication -p $PORT  -P pager=off -c "$sql;"
...
sudo -u postgres psql -d  replication -p $PORT  -P pager=off -c "$sql;"
sql="GRANT SELECT ON TABLE public.wal1 TO  $REPL_USER;"
sudo -u postgres psql -d  replication -p $PORT  -P pager=off -c "$sql;"
sql="GRANT SELECT ON TABLE public.logical1 TO  $REPL_USER;"
sudo -u postgres psql -d  replication -p $PORT  -P pager=off -c "$sql;"
sql="ALTER DEFAULT PRIVILEGES IN SCHEMA public
